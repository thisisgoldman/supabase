...
[Sequin](https://sequinstream.com/) streams changes from your Supabase database to destinations like SQS, HTTP webhooks, TypeSense, and Redis. Sequin guarantees delivery of every change, in a strict order, with real-time speed. It natively integrates with Supabse and supports the default IPv6 networking. It's built to power use cases like audit logging, replicating with a cache, maintaining a search index, or powering a microservices architecture.

Sequin has a [Supabase setup guide](https://sequinstream.com/docs/how-to/setup-supabase) to walk you through the steps of connecting your Supabase database to Sequin. TL;DR:
